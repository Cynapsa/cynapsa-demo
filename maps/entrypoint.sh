...
for secret_name in gemini_api_key google_maps_api_key; do
  if [ -s "$secret_source/$secret_name" ]; then
    cp "$secret_source/$secret_name" "$private_directory/$secret_name"
    chmod 600 "$private_directory/$secret_name"
  fi
done
if [ -n "${DEMO_GEMINI_API_KEY:-}" ]; then
  printf '%s' "$DEMO_GEMINI_API_KEY" > "$private_directory/gemini_api_key"
  chmod 600 "$private_directory/gemini_api_key"
fi
if [ -n "${DEMO_GOOGLE_MAPS_API_KEY:-}" ]; then
  printf '%s' "$DEMO_GOOGLE_MAPS_API_KEY" > "$private_directory/google_maps_api_key"
  chmod 600 "$private_directory/google_maps_api_key"
fi
unset DEMO_GEMINI_API_KEY DEMO_GOOGLE_MAPS_API_KEY
